...
function Entrenar() {
  ...
  return (
    <div className="space-y-5">
      ...
      {generated && generated.format !== "clasico" ? <WodRunner session={generated} /> : ((generated?.focus === "cardio" && generated.exercises.length === 0) || (!generated && mode === "cardio")) ? <CardioForm initialSession={generated ?? undefined} /> : <GymSession initialSession={generated ?? undefined} />}
...
function GeneratedPreview({ session, onStart, onAgain }: { session: GeneratedSession; onStart: () => void; onAgain: () => void }) {
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      {session.exercises.length > 0 && <div className="card"><h2 className="section-title">{session.format === "clasico" ? "Plan de trabajo" : "Estaciones del WOD"}</h2><ol className="mt-3 space-y-2">{session.exercises.map((exercise, index) => <li key={`${exercise.name}-${index}`} className="flex items-center gap-3 rounded-xl bg-base-800 p-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-accent/15 text-xs font-bold text-accent">{index + 1}</span><span className="min-w-0 flex-1 truncate text-sm font-semibold">{exercise.name}</span><span className="text-xs text-slate-400">{session.stations?.[index]?.target ?? `${exercise.sets.length} series`}</span></li>)}</ol></div>}
